  ...
  return (
    <>
      ...
      <section className="section">
        <a className="container opportunity opportunity--link" href="#engineering-manager-job-description" aria-label={`Read the full job description for ${careerOpening.title}`}>
      ...
      <section className="section section--white" id="engineering-manager-job-description">
        <div className="container job-description">
          <div className="job-description__intro">
            ...
            <div className="prose-large">{careerOpening.summary.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div>
          </div>
          <div className="job-description__body">
            <div className="section-header"><p className="eyebrow">Key responsibilities</p><h2>What you will own</h2><p>Open each area to review the responsibilities in detail.</p></div>
      ...
      <section className="section section--blue-grey">
        <div className="container">
          <div className="section-header"><p className="eyebrow">Experience and capability</p><h2>Technical skills for the role</h2></div>
          <div className="job-requirements-grid">
            <section><h3>Required technical skills</h3><ul className="job-list">{careerOpening.requiredSkills.map((skill) => <li key={skill}><Check size={17} />{skill}</li>)}</ul></section>
            <section><h3>Preferred skills</h3><ul className="job-list">{careerOpening.preferredSkills.map((skill) => <li key={skill}><Check size={17} />{skill}</li>)}</ul></section>
          </div>
          <div className="success-measures">
            <div><p className="eyebrow">Key success measures</p><h2>What good looks like</h2></div>
